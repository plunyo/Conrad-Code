...
function numPlayers(playerNum)
    ...
    local yPosition = ySpacing * (playerNum - 1)
        + ySpacing / 2
        - gameConfig.winnerwinY / 10

    -- Create new player colliders based on playerNum
    if playerNum >= 1 and not p1 then
        p1 = world:newRectangleCollider(
            gameConfig.winnerwinX / 3 - player.sizeW / 2,
            yPosition,
            player.sizeW,
            player.sizeH
        )
    ...
    if playerNum >= 2 and not p2 then
        p2 = world:newRectangleCollider(
            2 * gameConfig.winnerwinX / 3 - player.sizeW / 2,
            yPosition,
            player.sizeW,
            player.sizeH
        )
    ...
    if playerNum >= 3 and not p3 then
        p3 = world:newRectangleCollider(
            gameConfig.winnerwinX / 2 - player.sizeW / 2,
            yPosition,
            player.sizeW,
            player.sizeH
        )
...
function love.load()
    ...
    gameConfig.winnerwinX, gameConfig.winnerwinY =
        love.window.getDesktopDimensions() -- Set winX and winY to desktop dimensions
    ...
    ground = world:newRectangleCollider(
        groundX,
        gameConfig.winnerwinY - 300,
        groundWidth,
        5
    )
...
function love.update(dt)
    ...
    if p1 then
        ...
        if
            p1X < 0
            or p1X + 50 > gameConfig.winnerwinX
            or p1Y < 0
            or p1Y + 50 > gameConfig.winnerwinY
        then
            playerState.p1 = 2
        end
    end
    ...
    if p2 then
        ...
        if
            p2X < 0
            or p2X + 50 > gameConfig.winnerwinX
            or p2Y < 0
            or p2Y + 50 > gameConfig.winnerwinY
        then
            playerState.p2 = 2
        end
    end
    ...
    if p3 then
        ...
        if
            p3X < 0
            or p3X + 50 > gameConfig.winnerwinX
            or p3Y < 0
            or p3Y + 50 > gameConfig.winnerwinY
        then
            playerState.p3 = 2
        end
    end
    ...
    if
        gameConfig.winner == "Player 1"
        or gameConfig.winner == "Player 2"
        or gameConfig.winner == "Player 3"
    then
