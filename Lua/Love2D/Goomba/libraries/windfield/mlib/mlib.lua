...
local function getClosestPoint(perpendicularX, perpendicularY, ...)
    ...
    elseif checkFuzzy(slope, 0) then -- Horizontal line
        ...
        local perpendicularIntercept =
            getYIntercept(perpendicularX, perpendicularY, perpendicularSlope)
...
local function getLineSegmentIntersection(x1, y1, x2, y2, ...)
    ...
    local slope2, intercept2 =
        getSlope(x1, y1, x2, y2), getYIntercept(x1, y1, x2, y2)
...
local function getSegmentSegmentIntersection(x1, y1, x2, y2, x3, y3, x4, y4)
    local slope1, intercept1 =
        getSlope(x1, y1, x2, y2), getYIntercept(x1, y1, x2, y2)
    local slope2, intercept2 =
        getSlope(x3, y3, x4, y4), getYIntercept(x3, y3, x4, y4)

    if
        ((slope1 and slope2) and checkFuzzy(slope1, slope2))
        or (not slope1 and not slope2)
    then -- Parallel lines
